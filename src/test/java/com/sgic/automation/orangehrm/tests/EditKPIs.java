 ...
 * @Author Jeyapriya
 */
public class EditKPIs extends TestBase {
    private static final Logger LOGGER = Logger.getLogger(EditKPIs.class);
    /**
     * testcase:edit KeyPerformanceIndicators
     * @param jobTitle
     * @param KPI
     * @param minRating
     * @param maxRating
     */
    @Test( groups = "PERFORMANCE",priority = 3,dataProviderClass = PerformanceTestData.class,
            dataProvider = "editKPIs",testName = "edit KeyPerformanceIndicators")
    public void editKIPs(String jobTitle,String KPI,String minRating,String maxRating ) {
        ...
        LOGGER.info("edit KPI change the job title to :"+jobTitle+", change the KPI to "+KPI+", change the minimum Rating to "+
                minRating+", change the maximum rating to  "+maxRating);
